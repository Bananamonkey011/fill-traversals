...
Animation FloodFilledImage::animate(unsigned frameInterval) {
  ...
  for (unsigned i = 0; i<traversals_.size(); i++) {
    ImageTraversal* trav = traversals_.at(i);
    ColorPicker* color = colorPickers_.at(i);
    for(const Point pt: *trav) {

      HSLAPixel& cur = png_->getPixel(pt.x, pt.y);
      cur = color->getColor(pt.x, pt.y);
      count++;
